#!/usr/bin/env python
# This file has been modified by aroi on 07/09/2024.
"""
...
AVCC = Struct(
    ...
    EmbeddedBitStruct(
        ...
        Padding(3, pattern=b'\x01'),
        "num_sps" / BitsInteger(5),
    ),
    "sps" / Default(PascalString(Int16ub), []),
    "num_pps" / Int8ub,
    "pps" / Default(PascalString(Int16ub), []),
    "raw_bytes" / GreedyBytes # if profile == 100, 110, 122 or 144
)
...
CompressorNameField = ExprAdapter(
    String(32, padchar=b" "),
    encoder=lambda obj, ctx: (b'\x00' + obj.encode('utf-8')).ljust(32, b' '),
    decoder=lambda obj, ctx: obj[1:].split(b'\x00', 1)[0].decode('utf-8').strip()
)

VendorField = ExprAdapter(
    String(4, padchar=b" "),
    encoder=lambda obj, ctx: (b'\x00' + obj.encode('utf-8')).ljust(4, b' '),
    decoder=lambda obj, ctx: obj.split(b'\x00', 1)[0].decode('utf-8').strip()
)

VideoSampleEntryBox = Struct(
    "version" / Default(Int16ub, 0),
    "revision" / Const(Int16ub, 0),
    "vendor" / Default(VendorField, b"brdy"),
    ...
    "compressor_name" / Default(CompressorNameField, ""),
    "depth" / Default(Int16ub, 24),
    "color_table_id" / Default(Int16sb, -1),
    "remaining_boxes" / LazyBound(lambda _: GreedyRange(Box))
)

XMLSubtitleSampleEntryBox = Struct(
    "namespace" / CString(encoding="utf-8"),
    "raw_bytes" / GreedyBytes
)
...
SampleEntryBox = PrefixedIncludingSize(Int32ub, Struct(
    ...
    Embedded(Switch(this.format, {
        b"ec-3": AudioSampleEntryBox,
        b"mp4a": AudioSampleEntryBox,
        b"enca": AudioSampleEntryBox,
        b"hvc1": VideoSampleEntryBox,
        b"avc1": VideoSampleEntryBox,
        b"encv": VideoSampleEntryBox,
        b"wvtt": Struct("children" / LazyBound(lambda ctx: GreedyRange(Box))),
        b"stpp": XMLSubtitleSampleEntryBox
...
Box = PrefixedIncludingSize(Int32ub, Struct(
    ...
    Embedded(Switch(this.type, {
        ...
        b"pasp": RawBox, # we don't need it
        b"btrt": RawBox, # we don't need it
        b"colr": RawBox, # we don't need it
        b"avcC": AVCC,
        b"hvcC": HVCC,
        # dash
